...
describe('plugin-owned SQLite worker',()=>{
  ...
  it.each([5,99])('rejects future schema %i without changing database bytes or journal format',async version=>{
  ...
  it('preserves a supported-schema backup with a readable header but corrupt later B-tree page',async()=>{
    ...
    const header=new DatabaseSync(filename,{readOnly:true});try{expect(header.prepare('PRAGMA user_version').get()).toEqual({user_version:4});}finally{header.close();}
