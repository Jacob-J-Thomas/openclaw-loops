...
- Merge only after current applicable checks pass, review findings are resolved and the reviewed patch matches the final candidate. Re-run the metadata guard immediately before merge. Verify main and issue closure afterward; accept parents separately.
