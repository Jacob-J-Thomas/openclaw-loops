...
export async function verifyMissingSessionAction(requestAction){
  const response=await requestAction();
  assert(response&&typeof response==='object'&&!Array.isArray(response),'Missing-session action returned a malformed response.');
  assert.equal(response.ok,false,'Missing-session action unexpectedly succeeded.');
  assert.equal(response.code,'SESSION_REQUIRED','Missing-session action did not return the fixture identity denial.');
  return {label:'missing-session-action',allowed:false,hostDenied:false,code:'SESSION_REQUIRED'};
}
...
async function main(){
  ...
  try{
    ...
    receipt.observations.push(await verifyMissingSessionAction(()=>request(privileged,'plugins.sessionAction',{pluginId,actionId,payload:{operation:'service'}})));
