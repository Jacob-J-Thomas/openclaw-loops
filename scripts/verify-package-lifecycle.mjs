...
const previousSource=process.env.LOOPS_LIFECYCLE_PREVIOUS_REF;
...
const raw=pathResolve('.dev-profile',`package-lifecycle-evidence-${randomUUID()}`);
const profile=pathResolve('.dev-profile',`package-lifecycle-${randomUUID()}`);
...
const hosts={previous:host('previous'),current:host('current')};
...
const previousSha=hash(previous),currentSha=hash(current);
...
let client,completed=false,phase='archive-validation';
try{
  ...
  await client?.stopAndWait().catch(()=>{});await stop().catch(()=>{});if(completed)rmSync(profile,{recursive:true,force:true});
}
