...
import {observeGatewayReadiness} from './gateway-readiness-evidence.mjs';
...
const waitForListening=async()=>{const deadline=Date.now()+15000;let lastError;while(Date.now()<deadline){const child=childState.child;if(!child||child.exitCode!==null||child.signalCode!==null)throw Error('Gateway exited before listening.');try{await probe();return;}catch(error){lastError=error;await pause(100);}}readiness=await observeGatewayReadiness({child:childState.child,probe:async timeoutMs=>{try{await probe(timeoutMs);return true;}catch{return false;}},startedAt:gatewayStartedAt,restartOrdinal,remainingMs:()=>45000,pause});throw Error(`Gateway did not listen: ${String(lastError?.message??lastError)}`);};
...
const pluginState=pathResolve(profile,'state','loops-poc');
  ...
  writeLifecycleFailureReceipt(evidence,phase,error,{previous:{source:previousSource,sha256:previousSha,hostVersion:hosts?.previous?.version},current:{sha256:currentSha,hostVersion:hosts?.current?.version}},readiness);
