...
Advanced is available on standalone and Repeat-body inference nodes. Temperature and output tokens are **advisory** on the current public SDK. Reasoning is normalized by OpenClaw; when no node override is authored, the #201 candidate leaves it to the selected runtime rather than copying session reasoning. Portable fields for top_p, top_k, min_p, typical_p, frequency/presence/repetition penalty, seed and stop sequences are preserved by the definition contract but displayed as unsupported through the current isolated completion API. An unset unsupported field does not prevent execution; an explicit override fails preflight with the reason. Zero is retained, never treated as inheritance.
