...
Complete stable standalone Loops 1.0: all R01–R25 requirements and P0–P6 delivery packages, including resumed accessibility qualification, publication and fresh public-registry installation/migration. Preserve the nine-node palette, full agent lifecycle control, Advanced settings and migration identity. X01–X29 and SDK05 remain expansion after 1.0. The plugin repository and package do not change OpenClaw core; authorized upstream contributions live in separate checkouts and issue-linked PRs. The latest owner instruction decides scope; a label or old issue description does not expand it.

The owner also authorizes contributions addressing required OpenClaw API gaps, maintained in separate upstream checkouts and issue-linked PRs. This repository and package remain exclusively an external plugin. Consume only supported APIs in declared released host versions; do not publish host patches, a private runtime fork, provider clients or personal profiles here. Refresh current issue/PR ownership and scope before contributing; preserve another contributor's implementation and obtain maintainer agreement where upstream requires a new API design decision. Automated triage is not maintainer design approval, and an upstream merge is not a released plugin dependency.
...
The stable 1.0 goal completes only when the entire admitted release inventory passes, required upstream contracts are available in a tested released host, all applicable candidates are verified and merged, and the published 1.0 artifact passes fresh public-registry installation and migration. Missing public contracts or pending maintainer/release decisions are external dependencies, not completed requirements. Continue independent work while those dependencies remain; preserve their concrete exit evidence. Scoped plugin hardening or upstream PR submission may be accepted separately without declaring the release complete. Historical accessibility deferral is superseded by the owner's resumed R23 instruction.
