...
The guard discovers formal Codex reviews and automatic-review summaries, including running or clean automatic reviews with no formal submission. Mirror completed automatic reviews as `<!-- loops-review-auto:<review ID> -->` (use the summary comment ID when there is no formal review). After an explicitly requested review returns, record `<!-- loops-review-result:<request comment ID>:<review ID> -->` alongside its mirrored request, using the observed request/result links. Each request can reconcile only one result. Unpaired results conservatively count as additional runs; do not infer pairing from timestamps when automatic and manual work overlap. A clean manual response without a formal submission is already represented by its request. Wait for active reviews instead of submitting overlapping duplicate requests.
