...
| SDK03 | Current command/tool/action identity and session lookup remain typed. The opt-in v2 tool factory carries `assertInvocationCurrent`; Loops' Memory integration registers its declared native tools through that public route and checks it through awaited write settlement. The new `registerGatewayAccessPolicy` is available but unused by Loops. | Original-person profile revocation after a UI action returns, independent principal/team and newly selected target authority, reassignment and cross-session recovery. |
