...
- Current candidate peer: **OpenClaw 2026.9.4**. Local ordinary installation, public-command and matched 2026.9.3-to-2026.9.4 lifecycle qualification pass on Node 24.16.0; current-head hosted platform qualification remains required before acceptance.
- Published upstream release under qualification: [v2026.9.4](https://github.com/openclaw/openclaw/releases/tag/v2026.9.4), published September 11, commit `3a9d69db306cd7f081e06254cb89c4bcc14a7107`.
...
against the pinned **2026.9.4** peer. It makes no runtime or provider call.
...
**2026.9.4** peer without a runtime or provider call. A command context has an
...
checks the pinned **2026.9.4** public `runtime.subagent` contract for a named
...
| SDK06 | Cancellation of a durable host run without deleting its session; observable physical settlement and ownership enforcement. | Pinned 2026.9.4 publicly exposes `runtime.tasks.runs.bindSession`, lookup and `cancel({taskId,cfg})`; ordinary single-completion `AbortSignal` support also exists. The retained 2026.9.3 authenticated session-action probe bound its actual session/agent and found no task matching the returned subagent `runId` during its two-second lookup, so it did not call cancel or repeat cancel. The later host wait timed out with `providerStarted`, `paused` and `not-visible`; the returned session identity was the requester session and inspection saw two messages. | This qualifies names and one bounded availability observation only: it does not show that a task never appears, that another public route is absent, that inference completed, or that timeout/cancellation physically settled a provider. If durable runner selection proceeds, verify ownership/mapping races, cancel/repeat-cancel, cleanup, restart and transcript preservation without deleting a session. |
