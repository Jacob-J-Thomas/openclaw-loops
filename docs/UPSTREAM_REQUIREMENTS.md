...
| SDK03 | Authenticated initiating principal, current target-session read/control permission, permitted agent inventory and history/recovery across reset. | Pinned 2026.9.5 public consumer [SDK03](https://github.com/Jacob-J-Thomas/openclaw-loops/blob/main/test/contracts/sdk03-consumer.ts) qualifies host-attested current command/tool/session-action identity and ordinary current-session `getSessionEntry` lookup. The named feature-plugin identity surface is unchanged from 2026.9.4. Exported transcript helpers operate on caller-supplied scope; the host read fence protects an already-admitted current turn, not a plugin-chosen target. #115902 covers approval attribution; #127977/PR #127982 cover plugin-scoped session state, not authorization. | Reuse current-session evidence only: #47/#163 prove current non-main admission, foreign/read-only denial, reset retaining session ID/history, and delete/recreate denying the old run. Obtain a documented public host decision for a newly selected target, then test independently authenticated owner/member/read-only/other-session/reset/reassignment/recovery cases. Do not build a second identity or team-permission system inside Loops. |
...
| SDK06 | Cancellation of a durable host run without deleting its session; observable physical settlement and ownership enforcement. | Pinned 2026.9.5 retains synchronous `runtime.tasks.runs.bindSession`, lookup and `cancel({taskId,cfg})`, and adds persisted asynchronous task reads through `runtime.tasks.async.runs`, including owner-bound `resolve(runId)`. Ordinary single-completion `AbortSignal` support also exists. The retained 2026.9.3 authenticated session-action probe bound its actual session/agent and found no task matching the returned subagent `runId` during its two-second lookup, so it did not call cancel or repeat cancel. The later host wait timed out with `providerStarted`, `paused` and `not-visible`; the returned session identity was the requester session and inspection saw two messages. | Compile acceptance of async lookup is not runtime mapping or settlement evidence. It does not show when a task appears, that inference completed, or that timeout/cancellation physically settled a provider. If durable runner selection proceeds, verify ownership/mapping races, cancel/repeat-cancel, cleanup, restart and transcript preservation without deleting a session. |
...
| [#120139 — stop a plugin run without destroying its session](https://github.com/openclaw/openclaw/issues/120139) | Issue remains open. [PR #120140](https://github.com/openclaw/openclaw/pull/120140), author `amoghasgekar`, head `9f3bb9374a200931b76947a8411f66f1a6c12c6a`, closed unmerged for inactivity on September 15. The 2026.9.3 public task-run cancellation path corrects the earlier blanket claim that public abort was absent. | Refresh issue/PR disposition and contributor ownership before any core contribution. Supply ownership-negative, task/run mapping, stop/wait settlement, repeated cancel, restart and transcript-preservation cases; keep runtime changes out of this repo. Dependency is conditional on choosing durable subagent execution for an inference attempt. |
...
| [#127561 — configured top_p silently dropped](https://github.com/openclaw/openclaw/issues/127561) | Maintainer-authored issue; reviewed source path projects `topP` but omits the accepted `top_p` spelling. Existing open [PR #148538](https://github.com/openclaw/openclaw/pull/148538), head `565e9e1aeb5624cf3a1e3e2414a02d77620b68b2`, owns the implementation. The broader per-call runtime contract remains narrow in 2026.9.5. | **Review and support the existing owner; do not duplicate the implementation.** Qualify normalization at the shared boundary, override precedence/provider filtering, both OpenAI transports and explicit zero. This addresses inherited configuration only; SDK per-call forwarding and discovery require separate scope. |
