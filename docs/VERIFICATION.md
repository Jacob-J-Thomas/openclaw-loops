...
[Bolt #207](https://github.com/Jacob-J-Thomas/openclaw-loops/issues/207) retains the historical candidate's source/archive, installation/readback, static validation and dry-run evidence. [Bolt #224](https://github.com/Jacob-J-Thomas/openclaw-loops/issues/224) records the standalone 1.0 publication-preparation source and archive identity. Included documentation and release metadata change package bytes even when runtime payloads are identical. The current feature artifact also contains expansion changes, so its source/archive identity and acceptance must be recorded separately; #224 does not qualify those bytes. [Bolt #76](https://github.com/Jacob-J-Thomas/openclaw-loops/issues/76) owns actual verified publication and fresh public-registry installation.
