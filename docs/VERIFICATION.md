...
<a id="unpublished-100-candidate"></a>

## 1.0.0 candidate qualification

Version 1.0.0 has completed the accepted source/package/runtime qualification needed for publication preparation. The following immutable receipt identifies the preceding tested candidate baseline; later documentation/package revisions have their own archive identities and member comparisons on [Bolt #224](https://github.com/Jacob-J-Thomas/openclaw-loops/issues/224). Registry availability, publication hash and fresh public installation/migration are established only by [Bolt #76](https://github.com/Jacob-J-Thomas/openclaw-loops/issues/76).
...
The [candidate receipt](https://github.com/Jacob-J-Thomas/openclaw-loops/issues/207#issuecomment-5764565271) is supplemented by accepted [#223](https://github.com/Jacob-J-Thomas/openclaw-loops/issues/223#issuecomment-5771153194): 659 source tests, 86 extracted-package tests, 22 mounted checks and all four platform jobs passed. The accepted repair merge is PR #222 at `56c9d0351b69c5667cd55042c6d149db6e8e8281`. These receipts qualify the preceding runtime/package bytes; #224's doc/package repack has a new identity, and #76 alone establishes registry publication and fresh installation.
...
The final bounded review repair at `cee6f8f5964c911dcf308f99ce6d918a5c9d148e` also passes 646 local source tests and 86 package tests. Its before/after regression proves a failed overridden-agent completion reports the same model that was dispatched. That error-attribution-only delta does not change the previously observed successful runtime paths; its fresh platform/review receipts are retained on Bolt #201.

The candidate uses an explicit node model first, otherwise the selected agent's configured model and runtime-default reasoning through public-plugin completion. Newly admitted runs pin overridden-agent defaults, including Repeat body inference. Six real API/command/tool cases and two native editor runs exercised Codex/default and explicit Ollama paths. The repaired installed runtime also passed a fresh Codex/default completion and an Ollama Wait/restart test: after admission, the selected agent's configured default changed to Codex, but the resumed loop returned the exact marker from its saved Ollama model. No new conversation account/reasoning override was copied; existing legacy pins and host policy remain effective.

The 1.0 preparation additionally exercised the combined nine-node graph with three real Codex completions, two refinement iterations, concurrent unpublished draft editing, a pinned published revision, restart-preserved Wait and Human review, native approval, and complete result retrieval. A separate typed-false path reached Fail without inference; it is not evidence of Human-review rejection. The agent-default repair changed server runtime after that combined test, so its focused real restart proof and full regression/platform checks qualify that delta explicitly. These outcomes are included in the accepted #223 qualification receipt.

[Bolt #207](https://github.com/Jacob-J-Thomas/openclaw-loops/issues/207) retains the preceding candidate's source/archive, installation/readback, static validation and dry-run evidence. [Bolt #224](https://github.com/Jacob-J-Thomas/openclaw-loops/issues/224) records the final publication-preparation source and archive identity. Use #224 for this repacked artifact: included documentation and release metadata change package bytes even when runtime payloads are identical. [Bolt #76](https://github.com/Jacob-J-Thomas/openclaw-loops/issues/76) owns actual verified publication and fresh public-registry installation.

The host-context stress work in [#129](https://github.com/Jacob-J-Thomas/openclaw-loops/issues/129) and [#199](https://github.com/Jacob-J-Thomas/openclaw-loops/issues/199) remains deferred host-owned SDK05 work. Manual compact/read worked on 2026.9.5; the long Ollama tool-conversation compaction failure remains an observed runtime limitation, with no plugin-owned defect or supported correction identified. The owner accepted practical manual VoiceOver navigation under [#185](https://github.com/Jacob-J-Thomas/openclaw-loops/issues/185); that scope does not claim full speech or WCAG certification.
...
These consumers make no runtime, provider, Gateway, delivery or cancellation call. Source and extracted-package checks qualify the public type/package boundary only. Separate local runtime evidence records an ordinary unmodified-host 9.4-to-9.5 upgrade with all 27 installed package members compared, unchanged protected settings, preserved revisions and complete previous outputs. Fresh Codex command/tool and Ollama tool routes completed with actual history-to-run correlation; both outputs were inspected in the native editor. The private Codex probe required adapting to the host's new toolResult content shape, then verified the already completed run without duplicate inference. Applied reasoning and effective account remain unknown. [Bolt #186](https://github.com/Jacob-J-Thomas/openclaw-loops/issues/186) records final artifact identity, current matrix/lifecycle, review disposition and acceptance. Independent review also required the async task type consumer to reject undefined-only/never results; compile regression cases preserve that requirement.

## Historical conversation-model parity qualification

This section preserves the earlier SDK acceptance harness. Automatic conversation-model/account inheritance is now post-1.0; the current explicit-model/configured-default contract and real evidence are described above. This historical harness is not an additional 1.0 release gate.
