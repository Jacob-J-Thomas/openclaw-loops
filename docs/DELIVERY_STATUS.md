...
| R05 | Partial; expected-operation failures verified | Typed safe failures now traverse the public feature result contract and shared UI client. Live removed-history replay and keyboard conflict-merge checks pass in alpha.7. Unexpected host errors remain masked; full provider error/credential-redaction coverage remains. |
...
| R24 | Partial | Alpha.11 passes 143 source tests on macOS and a local Linux arm64 VM with Node 24.16.0/26.1.0, plus typecheck/lint/build and 23 extracted-package SDK tests. Compiled files match across platforms. Both installed dev profiles match the archive; every application table was unchanged by upgrade. Fresh user commands and agent tools executed the same saved revision on Codex and Ollama, with exact input/run-ID assertions and native UI inspection. Real filesystem-full recovery passed on both Linux Node versions. Hosted CI, x64, deployment roles, broader resources/soak/upgrade acceptance remain. |
