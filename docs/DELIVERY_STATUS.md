...
| R13/R14/R16 | Partial | Configurable queued concurrency, physical cleanup accounting, no 20/50 record caps, paged Unicode outputs and history. Retention policy, backend pagination/scale and physical-failure soak remain. |
| R15 | Partial | V2 nested JSON, zero-input graphs, safe traversal, typed equality and larger explicit budgets. More operator-configurable budgets and schema validation remain. |
| R17 | Partial | 28 agent tools; real agent invocation and lifecycle tests; concrete output schemas now replace shape-only contracts. Command human review and all-path contract coverage remain. |
| R19–R21 | Partial | Undo/redo, node duplication, layout, binding picker, search, draft recovery, version controls, unpublished tests and explicit recovery. Conflict merging, independent persistent browser drafts and archive recovery implemented; live draft/archive journeys pass. Full history scale and remaining authoring/a11y journeys remain. |
...
| R24 | Partial | 96 source tests and installed alpha.4 artifact, package SDK checks, two isolated dev profiles, SQLite migration and real model calls. Ollama agent reporting passed with the same 4B model at 32K after a 16K overflow. Feature-matrix success/review/restart path passed with Sol. Deployment/OS/resource/soak/upgrade matrix remains. |
| R25 | Partial | MIT license, 27 dependency notices, contributor/deployment guides, pinned CI workflow, installable alpha tarball and upstream gap contracts. Public source/CI, dry run, publication and fresh registry install remain. |
