...
| R11/R12 | Partial; migration and crash recovery verified locally | SQLite worker/WAL/integrity/backup, exact legacy import, attempt evidence and explicit recovery ancestry. Alpha.10 fixes competing stale-lock reclaimers with a lifetime SQLite lease and verifies 12 engine SIGKILL checkpoints. Alpha.11 commits run-only checkpoints without retransmitting retained history; a 1,000-run synthetic fixture reduced mean execution time from 884 ms to 2.6 ms. Startup still loads complete state and commit barriers remain synchronous; lazy loading, durable wake and wider fault coverage remain. |
...
| R24 | Partial | Alpha.12 passes 153 source tests on macOS and a local Linux arm64 VM with Node 24.16.0/26.1.0, plus typecheck/lint/build and 25 extracted-package SDK tests. Compiled files match across platforms. Both installed profiles match the final archive; every application table was unchanged by both upgrade steps. Fresh commands and agent tools executed the same saved revision on Codex and Ollama, with exact input/run-ID assertions. Native UI inspection verifies the repaired error display and Advanced controls. Real filesystem-full recovery passed on both Linux Node versions. Hosted CI, x64, deployment roles, broader resources/soak/upgrade acceptance remain. |
