...
| R11/R12 | Partial; migration and crash recovery verified locally | SQLite worker/WAL/integrity/backup, exact legacy import, attempt evidence and explicit recovery ancestry. Alpha.10 fixes competing stale-lock reclaimers with a lifetime SQLite lease, preserves ownership until worker termination and verifies 12 engine SIGKILL checkpoints. Recovered cancellation no longer retains an obsolete cleanup flag. Store still loads complete state and waits at synchronous commit barriers; scalability, durable wake and wider fault coverage remain. |
...
| R24 | Partial | Alpha.10 passes 141 source tests on macOS Node 24.16.0 and 26.1.0, typecheck/lint/build and 23 extracted-package SDK tests. Both installed dev profiles match the archive; every application table was unchanged by upgrade. Fresh user commands and agent tools executed the same saved revision on Codex and Ollama, with exact input/run-ID assertions and native UI inspection. Earlier feature-matrix success/review/restart proof remains. Deployment/OS/resource/soak/upgrade matrix remains. |
