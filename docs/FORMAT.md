...
The editor's **Text or binding / JSON value** selector makes the distinction explicit. Switching existing text to JSON quotes it as a string rather than silently changing its type. Unfinished JSON source is retained in browser recovery and saved drafts. Validation, publication and unpublished testing reject consumed invalid JSON; an unused `truthy` right operand remains unused. Drafting does not replace an enabled publication. Tools and JSON commands use the same value form. Export/import, duplication and version restoration preserve it. Cloning and importing preserve the source format and execution limits while assigning a new identity and revision zero; blank workflows and the built-in example templates start with v2 and current operator defaults. Existing v1 nodes continue to accept only strings; **Use version 2 in this draft** is an explicit, undoable format change that does not modify pinned runs or automatically publish the draft.
...
Agent `loops_create` accepts editable `definition` fields and assigns `id` and `revision`. Omit `schemaVersion` for the v2 default, or explicitly supply `3` for shared context. Its optional `enabled` flag defaults to **true**; use `false` for a draft. `loops_edit` takes `id`, `expectedRevision`, a nonempty `changes` object and optional `enabled`; omitting schemaVersion preserves the saved format. Omission preserves the saved loop's activation; true publishes the new revision and false saves a draft. Arrays/objects replace whole fields. Enabled saves validate topology and host access before committing anything. Draft saves return validation issues and remain non-runnable.
