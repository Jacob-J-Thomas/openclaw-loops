...
export function ContextLifecycleEditor({value,onChange,loadCapabilities}:{value:ContextLifecycle;onChange:(value:ContextLifecycle)=>void;loadCapabilities:(settings:InferenceSettings)=>Promise<InferenceCapabilities>}){
  ...
  return <section aria-label="Context lifecycle settings"><p className="lp-hint">Each operation uses only these explicit JSON Pointer paths. Compact and reset retain a durable before-state record; summaries send only this declared selection to the configured completion adapter.</p>
    <label className="lp-field"><span>Operation</span><select value={value.operation} onChange={event=>{const operation=event.target.value as ContextLifecycle['operation'];const next={...value,operation};if(operation!=='inject')delete next.value;if(operation==='summarize'||operation==='compact')delete next.source;onChange(next);}}>{operations.map(operation=><option key={operation} value={operation}>{operation}</option>)}</select></label>
    ...
    {!['summarize','compact'].includes(value.operation)&&<><label className="lp-field"><span>Source</span><select aria-label="Lifecycle source" value={source} onChange={event=>{const kind=event.target.value;const next={...value};if(!kind)delete next.source;else {delete next.value;next.source=kind==='initial'?{kind:'initial'}:{kind:'retained',sourceId:'',format:'snapshot'};}onChange(next);}}><option value="">{value.operation==='inject'?'Authored value':'Choose a source'}</option><option value="initial">Initial input snapshot</option><option value="retained">Retained source ID</option></select></label>
