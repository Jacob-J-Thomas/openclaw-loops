...
export const ContextStateSchema=Type.Object({version:Type.Integer({minimum:0}),value:Type.Record(Type.String(),Type.Unknown()),journal:Type.Array(ContextJournalEntrySchema)},strict);
export type ContextState={version:number;value:Record<string,Json>;journal:ContextJournalEntry[]};
...
export function applyContextPatch(state:ContextState,nodeId:string,config:ContextNodeConfig|undefined,output:Json,bind:(value:NodeValue)=>Json,digest:(value:string)=>string):ContextState{
  ...
  const serialized=JSON.stringify(source),baseVersion=state.version,next={version:baseVersion+1,value,journal:[...state.journal,{nodeId,baseVersion,version:baseVersion+1,mode:patch.mode,target,source:structuredClone(patch.source),valueSha256:digest(serialized),valueBytes:bytes(serialized)}]};
...
export function assertContextState(value:unknown):asserts value is ContextState{
  ...
  const state=value as ContextState;let previous=0;for(const entry of state.journal){
    ...
    if(entry.baseVersion!==previous||entry.version!==previous+1)throw requestError('Invalid saved context journal order.');previous=entry.version;
  }
