...
const plugin=defineFeaturePlugin({contract,name:'Loops',description:'Inspectable, bounded loops shared by native UI and chat.',setup(api,events){
  ...
  api.registerService({id:'loops-poc-store',start(){if(!engines.has(stateFile))engines.set(stateFile,new Engine(new SqliteStorage(join(api.runtime.state.resolveStateDir(),'loops-poc','loops.sqlite'),stateFile),bridge.host,{concurrency:typeof api.pluginConfig?.maxConcurrentRuns==='number'?api.pluginConfig.maxConcurrentRuns:1,onChange:()=>events.emit('changed',{})}));},async stop(){await engines.get(stateFile)?.close();engines.delete(stateFile);}});
  api.registerCommand({name:'loops',description:'Discover and execute saved bounded loops.',acceptsArgs:true,requireAuth:true,requiredScopes:['operator.write'],
    ...
    async handler(command){try{const actor=bridge.actor({source:'command',command,api}),parsed=parseCommand(command),e=service();
      if(parsed.op==='list')return {text:JSON.stringify(e.list(actor),null,2)};
      const run=parsed.op==='run'?await e.run(actor,parsed.slug,parsed.input,parsed.requestId):parsed.op==='status'?e.status(actor,parsed.runId):parsed.op==='resume'?await e.resume(actor,parsed.runId):e.cancel(actor,parsed.runId);
...
Object.assign(plugin.configSchema.jsonSchema??={}, {type:'object',additionalProperties:false,properties:{maxConcurrentRuns:{type:'integer',minimum:1,default:1,description:'Maximum executing/settling runs. Additional requests queue. Keep 1 for a 16 GB local-model machine.'}}});
plugin.configSchema.parse=(value)=>{const config=value??{};if(typeof config!=='object'||Array.isArray(config)||Object.keys(config).some(key=>key!=='maxConcurrentRuns'))throw new Error('Unknown Loops configuration field.');const concurrency=(config as {maxConcurrentRuns?:unknown}).maxConcurrentRuns;if(concurrency!==undefined&&(!Number.isInteger(concurrency)||Number(concurrency)<1))throw new Error('maxConcurrentRuns must be a positive integer.');return config;};
