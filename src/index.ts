...
const plugin=defineFeaturePlugin({contract:wireContract,name:'Loops',description:'Inspectable, bounded loops shared by native UI and chat.',setup(api,events){
  ...
  const handlers:FeatureHandlers<typeof contract> = {
    ...
    package_export:(p,c)=>service().invoke('packageExport',bridge.actor(c),p.id,p.bindings),package_preview:(p,c)=>service().invoke('packagePreview',bridge.actor(c),p.package,p.environment),package_import:(p,c)=>service().invoke('packageImport',bridge.actor(c),p.package,p.environment,p.digest,p.libraryDigest,p.enabled),
