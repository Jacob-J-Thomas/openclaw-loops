...
export function RunInspection({run,onSelectRun,onExport,onReadSource,onLookupMemoryMutation}:{run:Run;onSelectRun:(id:string)=>void;onExport:(run:Run)=>void;onReadSource?:(source:NonNullable<Run['context']>['sources'] extends (infer T)[]|undefined?T:never)=>Promise<unknown>;onLookupMemoryMutation?:(nodeId:string,mutationId:string)=>Promise<unknown>}){
  ...
  const [sourceDocument,setSourceDocument]=useState<{id:string;value:unknown}|null>(null),[sourceError,setSourceError]=useState(''),[sourceLoading,setSourceLoading]=useState('');
  const [memoryReceipt,setMemoryReceipt]=useState<{id:string;value:unknown}|null>(null),[memoryError,setMemoryError]=useState('');
  ...
  const memoryAttempt=selected?.kind==='memory'?[...run.trace].reverse().find(item=>item.nodeId===selected.id&&item.memoryMutationId):undefined;
  return <section className="lp-executed-workflow" aria-label="Executed workflow">
    ...
    {run.context?.sources?.length? <details className="lp-context-sources"><summary>Context lifecycle evidence ({run.context.sources.length})</summary><p className="lp-hint">Each source is owner-scoped immutable evidence captured before the operation. Compact and reset records identify the exact paths that changed.</p>{run.context.sources.map(source=><details key={`${source.id}-${source.targetVersion}`}><summary>{source.operation} · v{source.sourceVersion} → v{source.targetVersion} · {source.id}</summary><pre>{display(source)}</pre>{onReadSource&&<button type="button" disabled={Boolean(sourceLoading)} onClick={()=>{setSourceError('');setSourceLoading(source.id);void onReadSource(source).then(value=>setSourceDocument({id:source.id,value})).catch(error=>setSourceError(error instanceof Error?error.message:String(error))).finally(()=>setSourceLoading(''));}}>Read retained source</button>}{sourceDocument?.id===source.id&&<PagedJson value={sourceDocument.value} label={`Retained source ${source.id}`}/>}</details>)}{sourceError&&<p role="alert">Retained source read failed: {sourceError}</p>}</details>:null}
    <details open><summary>Executed workflow (read-only)</summary><p className="lp-hint">This graph is the definition saved with this run. It does not edit or replace the current authoring draft.</p>
      ...
      {selected&&<div className="lp-run-node-detail"><h3>{selected.label}</h3><p className="lp-muted">{nodeContract(selected.kind).editor.title} · {selected.id}</p><h4>Executed settings</h4><pre>{settings(selected)}</pre>{memoryAttempt?.memoryMutationId&&<><h4>Memory mutation identity</h4><code>{memoryAttempt.memoryMutationId}</code>{onLookupMemoryMutation&&<button type="button" onClick={()=>{setMemoryError('');void onLookupMemoryMutation(selected.id,memoryAttempt.memoryMutationId!).then(value=>setMemoryReceipt({id:memoryAttempt.memoryMutationId!,value})).catch(error=>setMemoryError(error instanceof Error?error.message:String(error)));}}>Check mutation receipt</button>}{memoryReceipt?.id===memoryAttempt.memoryMutationId&&<PagedJson value={memoryReceipt.value} label="Memory mutation receipt"/>}{memoryError&&<p role="alert">{memoryError}</p>}</>}{output.route&&<><h4>Selected route</h4><p className="lp-route"><code>{output.route.port}</code>{output.route.caseId&&<> · case {output.route.caseId}</>} · {output.route.available===false?<span>observed value unavailable</span>:<>observed <code>{output.route.observed}{output.route.truncated?'…':''}</code></>}</p></>}{output.checkpoint&&<><h4>{output.checkpoint.label}</h4><pre className="lp-checkpoint-output">{display(output.checkpoint.value)}</pre></>}<h4>{output.label}</h4>{output.value===undefined?<p className="lp-muted">No output is available for this node in this run.</p>:<pre className="lp-full-output">{display(output.value)}</pre>}</div>}
    </details>
  </section>;
}
