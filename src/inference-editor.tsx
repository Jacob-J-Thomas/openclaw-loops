...
export function InferenceEditor({node,onChange,loadCapabilities}:{node:InferenceNode;onChange:(node:InferenceNode)=>void;loadCapabilities:(settings:InferenceSettings)=>Promise<InferenceCapabilities>}){
  ...
  return <>
    ...
    <label className="lp-field"><span>Structured generation</span><select value={node.structuredGeneration??''} onChange={e=>{const next={...node};if(e.target.value==='native')next.structuredGeneration='native';else delete next.structuredGeneration;onChange(next);}}><option value="">Default isolated completion</option><option value="native" disabled={!node.outputSchema||node.output!=='json'}>Provider-native JSON schema</option></select></label>
    {node.structuredGeneration==='native'&&<p className="lp-hint">Requires Validated JSON and an output schema. OpenClaw sends a provider-native format request; the provider may ignore it, so Loops validates every response before saving it.</p>}
