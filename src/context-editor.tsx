...
export function ContextEditor<T extends ContextConfigurable>({node,onChange,heading='Shared context'}:{node:T;onChange:(node:T)=>void;heading?:string}){
  ...
  return <section className="lp-context-editor" aria-label={heading}>
    ...
    {config&&<>
      ...
      {projection.mode==='consume'&&<div className="lp-context-paths"><p className="lp-hint">This node can bind only the selected paths as <code>{'{{context…}}'}</code>. Use RFC 6901 escapes: <code>~0</code> for <code>~</code>, <code>~1</code> for <code>/</code>.</p>
